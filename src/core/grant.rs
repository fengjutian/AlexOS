...
impl ResourceScope {
    pub fn exact(kind: ResourceKind, id: impl Into<String>) -> Self {
        Self { kind, id: id.into(), match_mode: ScopeMatch::Exact }
    }

    pub fn prefix(kind: ResourceKind, id: impl Into<String>) -> Self {
        Self { kind, id: id.into(), match_mode: ScopeMatch::Prefix }
    }

    fn matches(&self, kind: ResourceKind, id: &str) -> bool {
        self.kind == kind && match self.match_mode {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize, Deserialize)]
#[serde(rename_all = "kebab-case")]
pub enum GrantStatus { Active, Consumed, Revoked }
...
impl GrantStore {
    pub fn issue(&self, spec: GrantSpec) -> Result<String, GrantError> {
        ...
        if spec.expires_at_ms <= now {
            return Err(GrantError::InvalidSpec("expiry must be in the future"));
        }
        ...
        grants.insert(id.clone(), Grant {
            ...
            id: id.clone(), spec, issued_at_ms: now,
            status: GrantStatus::Active, revoked_reason: None,
        });
        Ok(id)
    }
    ...
    pub fn claim(&self, id: &str, claim: &GrantClaim<'_>) -> Result<(), GrantError> {
        ...
        let burn = grants.get(id).is_some_and(|grant| grant.spec.consume_on_attempt);
    ...
    pub fn revoke(&self, id: &str, reason: impl Into<String>) -> Result<usize, GrantError> {
        let mut grants = self.grants.lock().map_err(|_| GrantError::StorePoisoned)?;
        if !grants.contains_key(id) { return Err(GrantError::Missing); }
        let reason = reason.into();
        let mut targets = vec![id.to_owned()];
        let mut index = 0;
        while index < targets.len() {
            let parent = &targets[index];
            let children: Vec<_> = grants.iter()
    ...
    pub fn revoke_grantee(&self, grantee: &PrincipalId, reason: &str) -> usize {
        let ids = self.grants.lock().expect("grant store lock poisoned").iter()
            .filter(|(_, grant)| &grant.spec.grantee == grantee && grant.status == GrantStatus::Active)
            .map(|(id, _)| id.clone()).collect::<Vec<_>>();
        ids.iter().map(|id| self.revoke(id, reason).unwrap_or(0)).sum()
    }
...
fn validate_spec(spec: &GrantSpec) -> Result<(), GrantError> {
    ...
    if spec.resources.is_empty() || spec.resources.iter().any(|r| r.id.is_empty() || r.id.len() > 2_048) {
        return Err(GrantError::InvalidSpec("resource scopes are required"));
    }
    if spec.max_uses == Some(0) { return Err(GrantError::InvalidSpec("maxUses must be positive")); }
    Ok(())
}

fn validate_parent(grants: &BTreeMap<String, Grant>, parent_id: &str, child: &GrantSpec, now: u64) -> Result<(), GrantError> {
    ...
    if parent.spec.grantee != child.issuer
        ...
        || !child.resources.iter().all(|c| parent.spec.resources.iter().any(|p| p.contains(c)))
    ...
    for _ in 0..MAX_PARENT_DEPTH {
        ...
        cursor = grants.get(id).ok_or(GrantError::Missing)?.spec.parent_id.as_deref();
    }
    Err(GrantError::ParentDepth)
}

fn uses_attenuated(parent: Option<u32>, child: Option<u32>) -> bool {
    match (parent, child) { (Some(p), Some(c)) => c <= p, (Some(_), None) => false, _ => true }
}

fn validate_claim(grants: &BTreeMap<String, Grant>, id: &str, claim: &GrantClaim<'_>, now: u64) -> Result<(), GrantError> {
    ...
    if &grant.spec.grantee != claim.grantee
        ...
        || !grant.spec.resources.iter().any(|scope| scope.matches(claim.resource_kind, claim.resource_id))
...
fn validate_active(grant: &Grant, now: u64) -> Result<(), GrantError> {
    ...
    if grant.spec.expires_at_ms <= now { return Err(GrantError::Expired); }
    Ok(())
}
...
fn consume(grant: &mut Grant) {
    if let Some(remaining) = &mut grant.remaining_uses {
        *remaining = remaining.saturating_sub(1);
        if *remaining == 0 { grant.status = GrantStatus::Consumed; }
    }
}
...
mod tests {
    ...
    fn spec(issuer: PrincipalId, grantee: PrincipalId) -> GrantSpec {
        GrantSpec { issuer, grantee, parent_id: None, actions: BTreeSet::from(["mcp.invoke".into()]),
            resources: vec![ResourceScope::exact(ResourceKind::McpTool, "mcp://server/tool")],
            expires_at_ms: expires_after(Duration::from_secs(60)), max_uses: Some(1), session_id: None,
            generation: 1, consume_on_attempt: true }
    }
    ...
    fn one_shot_claim_is_atomic_under_concurrency() {
        ...
        let id = store.issue(spec(principal(crate::identity::PrincipalKind::System, "alexd"), app.clone())).unwrap();
        let barrier = Arc::new(Barrier::new(3));
        let handles: Vec<_> = (0..2).map(|_| {
            let store = store.clone(); let id = id.clone(); let app = app.clone(); let barrier = barrier.clone();
            thread::spawn(move || { barrier.wait(); store.claim(&id, &GrantClaim { grantee: &app, action: "mcp.invoke",
                resource_kind: ResourceKind::McpTool, resource_id: "mcp://server/tool", session_id: None, generation: 1 }) })
        }).collect();
        barrier.wait();
        assert_eq!(handles.into_iter().map(|h| h.join().unwrap().is_ok()).filter(|ok| *ok).count(), 1);
    }
    ...
    fn child_must_attenuate_and_parent_revocation_cascades() {
        ...
        let mut parent_spec = spec(system, app.clone()); parent_spec.max_uses = Some(3);
        ...
        let mut child_spec = spec(app, agent.clone()); child_spec.parent_id = Some(parent.clone());
        child_spec.expires_at_ms = parent_spec.expires_at_ms;
        let child = store.issue(child_spec).unwrap();
        let mut invalid = spec(parent_spec.grantee, agent); invalid.parent_id = Some(parent.clone());
