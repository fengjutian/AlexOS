...
// Generated from src/filesystem.ts. Run `npm run build` after changing source.
const root = path.resolve(process.env.ALEX_WORKSPACE ?? "workspace");
const input = readline.createInterface({ input: process.stdin });
function resolveWorkspacePath(value = ".") {
  const resolved = path.resolve(root, value);
  if (resolved !== root && !resolved.startsWith(`${root}${path.sep}`)) throw new Error("path escapes the workspace");
  return resolved;
}
...
async function handle(message) {
  if (message.method === "initialize") {
    reply(message.id, { result: { protocolVersion: "2025-03-26", capabilities: { tools: {} }, serverInfo: { name: "alex-filesystem", version: "0.1.0" } } });
    return;
  }
  ...
  let text;
  if (name === "read_text_file") text = await fs.readFile(resolveWorkspacePath(requestedPath), "utf8");
  else if (name === "list_directory") text = (await fs.readdir(resolveWorkspacePath(requestedPath))).join("\n");
