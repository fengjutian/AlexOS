...
当前基线（2026-08-25）：`cargo test -p alex --lib` 335 passed / 0 failed，SDK 测试 4 passed。
测试数量会随功能变化。CI 配置见 `.github/workflows/ci.yml`（Linux 跑 fmt/clippy/test/SDK，
Windows 跑 build/test/SDK 并上传 `alex.exe` 产物）。
