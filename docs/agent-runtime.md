...
- `agent.schedule(runId, scheduledAtMs)` persists a one-shot background execution time. The daemon scheduler atomically claims due runs, executes them without requiring an open WebView stream, and recovers pending schedules after restart. Scheduling is limited to one year ahead.
- App Manager exposes a dedicated approval queue with application, run, tool and arguments. Approve/deny actions are daemon-authoritative and remain subject to `agent.run` plus the run-generation checks.
